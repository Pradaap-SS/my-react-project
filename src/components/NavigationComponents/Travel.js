...
const Travel = () => {
  ...
  const [travelSection, setTravelSection] = useState(/* initial value */);
  ...
  const handleDeleteBlog = (topicName, blogTitle) => {
    ...
    if (isConfirmed) {
      fetch(`http://localhost:5000/delete-blog/${topicName}/${encodeURIComponent(blogTitle)}`, {
        ...
        .then((data) => {
          const updatedTopics = [...travelSection.topics];
          ...
          if (topicIndex !== -1) {
            ...
            setTravelSection((prev) => ({ ...prev, topics: updatedTopics }));
          } else {
  ...
  const TravelSection = topicsData.topics.find((topic) => topic.name === 'Travel');
  ...
  const { name: title, blogs } = TravelSection;

  return (
    <ThemeProvider theme={defaultTheme}>
      <CssBaseline />
      <Container maxWidth="lg">
        <Header title={title} sections={[TravelSection]} />
