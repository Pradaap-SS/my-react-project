...
const Alumni = () => {
  ...
  const [alumniSection, setalumniSection] = useState(/* initial value */);
  const topicName = "alumni"
  ...
  const handleDeleteBlog = (topicName, blogTitle) => {
    ...
    if (isConfirmed) {
      fetch(`http://localhost:5000/delete-blog/${topicName}/${encodeURIComponent(blogTitle)}`, {
        ...
        .then((data) => {
          const updatedTopics = [...alumniSection.topics];
          ...
          if (topicIndex !== -1) {
            ...
            setalumniSection((prev) => ({ ...prev, topics: updatedTopics }));
          } else {
  ...
  const AlumniSection = topicsData.topics.find((topic) => topic.name === 'Alumni');
  ...
  const { name: title, blogs } = alumniSection;

  return (
    <ThemeProvider theme={defaultTheme}>
      <CssBaseline />
      <Container maxWidth="lg">
        <Header title={title} sections={[alumniSection]} />
