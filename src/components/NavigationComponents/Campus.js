...
const Campus = () => {
  ...
  const [campusSection, setCampusSection] = useState(/* initial value */);
  const topicName = "Campus"
  ...
  const handleDeleteBlog = (topicName, blogTitle) => {
    ...
    if (isConfirmed) {
      fetch(`http://localhost:5000/delete-blog/${topicName}/${encodeURIComponent(blogTitle)}`, {
        ...
        .then((data) => {
          const updatedTopics = [...campusSection.topics];
          ...
          if (topicIndex !== -1) {
            ...
            setCampusSection((prev) => ({ ...prev, topics: updatedTopics }));
          } else {
  ...
  const CampusSection = topicsData.topics.find((topic) => topic.name === 'Campus');
  ...
  const { name: title, blogs } = CampusSection;

  return (
    <ThemeProvider theme={defaultTheme}>
      <CssBaseline />
      <Container maxWidth="lg">
        <Header title={title} sections={[CampusSection]} />
