...
const Culture = () => {
  ...
  const [cultureSection, setCultureSection] = useState(/* initial value */);
  const topicName = "Culture"
  ...
  const handleDeleteBlog = (topicName, blogTitle) => {
    ...
    if (isConfirmed) {
      fetch(`http://localhost:5000/delete-blog/${topicName}/${encodeURIComponent(blogTitle)}`, {
        ...
        .then((data) => {
          const updatedTopics = [...cultureSection.topics];
          ...
          if (topicIndex !== -1) {
            ...
            setCultureSection((prev) => ({ ...prev, topics: updatedTopics }));
          } else {
  ...
  const CultureSection = topicsData.topics.find((topic) => topic.name === 'Culture');
  ...
  const { name: title, blogs } = CultureSection;

  return (
    <ThemeProvider theme={defaultTheme}>
      <CssBaseline />
      <Container maxWidth="lg">
        <Header title={title} sections={[CultureSection]} />
