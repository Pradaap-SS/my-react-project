...
const Sports = () => {
  ...
  const [sportsSection, setSportsSection] = useState(/* initial value */);
  const topicName = "Sports"
  ...
  const handleDeleteBlog = (topicName, blogTitle) => {
    ...
    if (isConfirmed) {
      fetch(`http://localhost:5000/delete-blog/${topicName}/${encodeURIComponent(blogTitle)}`, {
        ...
        .then((data) => {
          const updatedTopics = [...sportsSection.topics];
          ...
          if (topicIndex !== -1) {
            ...
            setSportsSection((prev) => ({ ...prev, topics: updatedTopics }));
          } else {
  ...
  const SportsSection = topicsData.topics.find((topic) => topic.name === 'Sports');
  ...
  const { name: title, blogs } = SportsSection;

  return (
    <ThemeProvider theme={defaultTheme}>
      <CssBaseline />
      <Container maxWidth="lg">
        <Header title={title} sections={[SportsSection]} />
