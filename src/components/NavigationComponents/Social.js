...
const Social = () => {
  ...
  const [socialSection, setSocialSection] = useState(/* initial value */);
  const topicName = "Social"
  ...
  const handleDeleteBlog = (topicName, blogTitle) => {
    ...
    if (isConfirmed) {
      fetch(`http://localhost:5000/delete-blog/${topicName}/${encodeURIComponent(blogTitle)}`, {
        ...
        .then((data) => {
          const updatedTopics = [...socialSection.topics];
          ...
          if (topicIndex !== -1) {
            ...
            setSocialSection((prev) => ({ ...prev, topics: updatedTopics }));
          } else {
  ...
  const SocialSection = topicsData.topics.find((topic) => topic.name === 'Social');
  ...
  const { name: title, blogs } = SocialSection;

  return (
    <ThemeProvider theme={defaultTheme}>
      <CssBaseline />
      <Container maxWidth="lg">
        <Header title={title} sections={[SocialSection]} />
