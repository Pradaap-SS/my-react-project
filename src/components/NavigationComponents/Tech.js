...
const Tech = () => {
  ...
  const [techSection, setTechSection] = useState(/* initial value */);
  const topicName = "Tech"
  ...
  const handleDeleteBlog = (topicName, blogTitle) => {
    ...
    if (isConfirmed) {
      fetch(`http://localhost:5000/delete-blog/${topicName}/${encodeURIComponent(blogTitle)}`, {
        ...
        .then((data) => {
          const updatedTopics = [...techSection.topics];
          ...
          if (topicIndex !== -1) {
            ...
            setTechSection((prev) => ({ ...prev, topics: updatedTopics }));
          } else {
  ...
  const TechSection = topicsData.topics.find((topic) => topic.name === 'Technology');
  ...
  const { name: title, blogs } = TechSection;

  return (
    <ThemeProvider theme={defaultTheme}>
      <CssBaseline />
      <Container maxWidth="lg">
        <Header title={title} sections={[TechSection]} />
